...
inline int ix_compare(const char* a, const char* b, ColType type, int col_len) {
    if (a == nullptr || b == nullptr || col_len <= 0) {
        throw InternalError("Invalid index comparison input");
    }
    switch (type) {
        case TYPE_INT: {
            if (col_len != static_cast<int>(sizeof(int))) {
        ...
        case TYPE_FLOAT: {
            if (col_len != static_cast<int>(sizeof(float))) {
...
inline int ix_compare(const char* a,
                      const char* b,
                      const std::vector<ColType>& col_types,
                      const std::vector<int>& col_lens) {
    if (col_types.size() != col_lens.size()) {
...
public:
    ...
    int get_size() const noexcept { return page_hdr->num_key; }

    void set_size(int size) { page_hdr->num_key = size; }

    int get_max_size() const noexcept { return file_hdr->btree_order_ + 1; }
    ...
    page_id_t get_page_no() const noexcept { return page->get_page_id().page_no; }

    PageId get_page_id() const noexcept { return page->get_page_id(); }

    page_id_t get_next_leaf() const noexcept { return page_hdr->next_leaf; }

    page_id_t get_prev_leaf() const noexcept { return page_hdr->prev_leaf; }

    page_id_t get_parent_page_no() const noexcept { return page_hdr->parent; }

    bool is_leaf_page() const noexcept { return page_hdr->is_leaf; }

    bool is_root_page() const noexcept { return get_parent_page_no() == INVALID_PAGE_ID; }
    ...
    char* get_key(int key_idx) const { return keys + static_cast<std::ptrdiff_t>(key_idx) * file_hdr->col_tot_len_; }

    Rid* get_rid(int rid_idx) const { return &rids[rid_idx]; }

    void set_key(int key_idx, const char* key) {
        memcpy(keys + static_cast<std::ptrdiff_t>(key_idx) * file_hdr->col_tot_len_, key, file_hdr->col_tot_len_);
    }
...
public:
    ...
    bool coalesce(IxNodeHandle** neighbor_node,
                  IxNodeHandle** node,
                  IxNodeHandle** parent,
                  int index,
                  Transaction* transaction,
...
private:
    ...
    void unpin_node(IxNodeHandle* node, bool dirty) const;
