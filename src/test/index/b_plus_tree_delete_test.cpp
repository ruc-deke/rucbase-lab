...
TEST_F(BPlusTreeTests, LargeScaleTest) {
    ...
    if (order >= 2 && order <= tree_->file_header_->tree_order_) {
        tree_->file_header_->tree_order_ = order;
    }
    int add_cnt = 0;
    int del_cnt = 0;
    ...
    while (add_cnt + del_cnt < scale) {
        ...
        double insert_prob = 1. - static_cast<double>(mock.size()) / (0.5 * scale);
        if (mock.empty() || dice < insert_prob) {
            // Insert
            int rand_key = rand() % scale;
            if (mock.contains(rand_key)) {  // 防止插入重复的key
                // printf("重复key=%d!\n", rand_key);
                continue;
            }
            Rid rand_val = {.page_no = rand(), .slot_no = rand()};
            printf("insert rand key=%d\n", rand_key);
            bool insert_ret = tree_->insert_entry((const char*)&rand_key, rand_val, txn_.get());  // 调用Insert
            ASSERT_EQ(insert_ret, true);
            ...
            size_t rand_idx = static_cast<size_t>(rand()) % mock.size();
            auto it = mock.begin();
            for (size_t k = 0; k < rand_idx; k++) {
                it++;
            }
            int key = it->first;
