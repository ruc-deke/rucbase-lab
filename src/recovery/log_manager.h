...
inline constexpr const char* LogTypeStr[] = {"UPDATE", "INSERT", "DELETE", "BEGIN", "COMMIT", "ABORT"};
...
public:
    InsertLogRecord() {
        ...
        log_tid_ = INVALID_TXN_ID;
        prev_lsn_ = INVALID_LSN;
        rid_ = {.page_no = INVALID_PAGE_ID, .slot_no = -1};
        table_name_ = nullptr;
        table_name_size_ = 0;
    }
    ...
    void deserialize(const char* src) override {
        ...
        size_t offset = static_cast<size_t>(OFFSET_LOG_DATA + insert_value_.size) + sizeof(int);
