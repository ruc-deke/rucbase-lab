...
public:
    ...
    void create_file(const std::string& filename, int record_size) {
        ...
        // 数据页布局：Page公共头 | RmPageHdr | bitmap | 定长记录槽。
        // 先忽略 bitmap，估算最多能放多少条记录；随后逐步减小，直到完整布局能放进一页。
        int num_records = (PAGE_SIZE - Page::OFFSET_PAGE_HDR - static_cast<int>(sizeof(RmPageHdr))) / record_size;
        while (true) {
