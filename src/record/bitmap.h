...
public:
    ...
    static void set(char* bm, int pos) {
        auto* bytes = reinterpret_cast<unsigned char*>(bm);
        bytes[get_bucket(pos)] |= get_bit(pos);
    }

    // pos位 置0
    static void reset(char* bm, int pos) {
        auto* bytes = reinterpret_cast<unsigned char*>(bm);
        bytes[get_bucket(pos)] &= static_cast<unsigned char>(~get_bit(pos));
    }

    // 如果pos位是1，则返回true
    static bool is_set(const char* bm, int pos) {
        const auto* bytes = reinterpret_cast<const unsigned char*>(bm);
        return (bytes[get_bucket(pos)] & get_bit(pos)) != 0;
    }
...
private:
    ...
    static unsigned char get_bit(int pos) {
        return static_cast<unsigned char>(BITMAP_HIGHEST_BIT >> static_cast<unsigned>(pos % BITMAP_WIDTH));
    }
};
