...
- 所有权只用四种语义，用类型表达，不要靠注释猜：

| 语义 | 类型 | 含义 |
| --- | --- | --- |
| 值 | `T`，访问器返回 `T` 或 `const T&` | 对象跟成员一起走 |
| 独占 | `std::unique_ptr<T>` 或 move-only RAII | 析构即释放 |
| 共享 | `std::shared_ptr<const T>` | 只读 AST 等确实需要共享的不可变对象 |
| 借用 | 必在用 `T&`；可空非拥有才用 `T*` | 指针必须写明有效期 |

- Lab 文档按实验讲义来写：说明任务、接口和提示。不要写维护者门禁、所有权术语或“不要打开某某头文件”。
